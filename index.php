...
.button-cluster button:nth-child(2n) {
background: #666;
}
...
// early exit for externs
if (isset($_GET['extern'])) {
    if ($_GET['extern'] == 'htmx.js') {
        header('Content-Type: text/javascript;charset=utf-8');
        echo $htmx;
        die(200);
    }

    if ($_GET['extern'] == 'styles.css') {
...
// Datastore
function __initialize_db(): SQLite3
{
    $fd = fopen(__FILE__, 'r');
    fseek($fd, __COMPILER_HALT_OFFSET__);
    $data = fread($fd, 10_000_000); // 10 MB size limit. If you exceed this, seek therapy.

    $tmpfs_name = tempnam("/tmp", "php");
    $tmpfs_fd = fopen($tmpfs_name, "w");
    fwrite($tmpfs_fd, $data);
    fclose($tmpfs_fd);

    $GLOBALS['tmpfs_name'] = $tmpfs_name;

    $db = new SQLite3($tmpfs_name);
    return $db;
}
function __close_db(SQLite3 $db): void {
    if (isset($GLOBALS['didclose'])) return;

    $db->close();

    $tmpfs_fd = fopen($GLOBALS['tmpfs_name'], 'r');
    $data = fread($tmpfs_fd, 10_000_000);
    fclose($tmpfs_fd);

    unlink($GLOBALS['tmpfs_name']);

    $this_script = fopen(__FILE__, 'r');
    $script_contents = fread($this_script, __COMPILER_HALT_OFFSET__);
    fclose($this_script);

    $this_script = fopen(__FILE__, 'w');
    fwrite($this_script, $script_contents);
    fwrite($this_script, $data);
    fclose($this_script);

    $GLOBALS['didclose'] = true;
}
$db = __initialize_db();
$db->exec("CREATE TABLE IF NOT EXISTS todo_items(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, content TEXT, done INTEGER)");

$headers = getallheaders();

function render_fragment_form(): string {
    global $base;
    return '<form hx-post="' . $base . '" id="create-item">
                <label>
                    <h3>Title:</h3>
...
function render_fragment_todo_item(string $title, string $content, string $id, int $done): string {
    global $base;
    $extra = '';
    if ($done > 0) {
        $extra = ' done';
    ...
    $extra_html = '';
    if ($done < 1) {
        $extra_html = '<button hx-patch="' . $base . '?item=' . $id . '">Mark this as done</button>
<button hx-get="' . $base . '?item=' . $id . '">Edit this</button>';
    }
    return '<div class="todo-item' . $extra . '" id="todo-item-'. $id . '" hx-target="this" hx-swap="outerHTML">
    <div>
    <h3>' . htmlspecialchars($title) . '</h3>
    <p>' . htmlspecialchars($content) . '</p></div><div class="button-cluster' . $extra . '">
    <button hx-delete="' . $base . '?item=' . $id . '">Delete this</button>
' . $extra_html . '</div>
</div>';
}

if (isset($headers['HX-Request'])) {
    if (isset($headers['HX-Trigger']) && $headers['HX-Trigger'] == 'create-item') {
        if (isset($_POST['title']) && isset($_POST['content'])) {
            $stmt = $db->prepare('INSERT INTO todo_items(title, content, done) VALUES(?, ?, 0)');
            $stmt->bindParam(1, $_POST['title']);
            $stmt->bindParam(2, $_POST['content']);

            $result = $stmt->execute();
            $lirid = $db->lastInsertRowID();

            $stmt = $db->prepare('SELECT * FROM todo_items WHERE id = ?');
            $stmt->bindParam(1, $lirid);
            $result = $stmt->execute();
            $row = $result->fetchArray();

            echo render_fragment_todo_item($row['title'], $row['content'], $row['id'], $row['done']);
            echo '<div class="todo-item" hx-target="this" hx-swap="outerHTML">';
            echo  render_fragment_form();
            echo '</div>';
        }
    }

    if (isset($headers['HX-Trigger']) && str_starts_with($headers['HX-Trigger'], 'update-item')) {
        if (isset($_POST['title']) && isset($_POST['content'])) {
            $stmt = $db->prepare('UPDATE todo_items SET title = ?, content = ? WHERE id = ?');
            $stmt->bindParam(1, $_POST['title']);
            $stmt->bindParam(2, $_POST['content']);
            $stmt->bindParam(3, $_GET['item']);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'DELETE') {
        $stmt = $db->prepare('DELETE FROM todo_items WHERE id = ?');
        $stmt->bindParam(1, $_GET['item']);
        $stmt->execute();
        echo '';
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PATCH') {
        $stmt = $db->prepare('UPDATE todo_items SET done = 1 WHERE id = ?');
        $stmt->bindParam(1, $_GET['item']);
        $stmt->execute();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        echo '<div class="todo-item" hx-target="this" hx-swap="outerHTML"><form hx-post="' . $base . '?item=' . $_GET['item'] . '" id="update-item' . $_GET['item'] . '">
                <label>
                    <h3>Title:</h3>
                    <input name="title" type="text" value="' . htmlspecialchars($row['title']) . '">
                </label>
                <label>
                    <h3>Content:</h3>
                    <textarea name="content">' . htmlspecialchars($row['content']) . '</textarea>
                </label>
...
<head lang="en">
    ...
    <meta charset="UTF-8">
    <link rel="stylesheet" href="<?= $base ?>?extern=styles.css">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <script src="<?= $base ?>?extern=htmx.js"></script>
</head>
<body>
    <div class="todo-holder">
        <?php
        $get_todos_query = $db->query('SELECT * FROM todo_items ORDER BY done DESC;');
        while ($row = $get_todos_query->fetchArray()) {
            ?><?= render_fragment_todo_item($row['title'], $row['content'], $row['id'], $row['done']) ?><?php
        }
        ?>
        <div class="todo-item" hx-target="this" hx-swap="outerHTML">
            <?= render_fragment_form(); ?>
        </div>
    </div>
</body>
<?php
